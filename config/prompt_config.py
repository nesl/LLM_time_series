...
class PromptConfigDTMF:
    ...
    def get_freq_pair_prompt(self, freq_pair, enable_map=False):
        """
        ...
        if enable_map:
            return (
                self.system_prompt + 
                f"""You are given a frequency pair of a tone. This tone represents a key (0-9, * or #). Here is the frequency pair: {freq_pair}.
                Please identify the key it refers to using the map below.""" + 
                self.get_dtmf_map() + 
                self.get_response_format_prompt(model="gpt-4o", enable_freq=False)
            )
        else:
            return (
                self.system_prompt + 
...
class PromptConfigHAR:
    ...
    def get_response_format_prompt(self, model):
        ...
        if model == "deepseek-reasoner":
            ...
            return """Please provide your answer in the following JSON structure:
            {
    ...
    def get_time_text_prompt(self, imu_data, model):
        ...
        return self.system_prompt + f"""The person's action belongs to one of the following categories: Still, Walking, Run, Car. The IMU data is given in the IMU coordinate frame. The three-axis accelerations, gyroscopes and magnetometers are given below.
            Accelerations:
            ...
            Could you please tell me what action the person was doing based on the given information and IMU readings? Please make an analysis step by step. """ + self.get_response_format_prompt(model)
    ...
    def get_time_text_fewshot_prompt(self, fewshot_data_dict, imu_data, model):
        ...
        return self.system_prompt + f"""The person's action belongs to one of the following categories: Still, Walking, Run, Car. Here are examples for each class:
        Still:
            Accelerations:
    ...
    def get_time_text_description_step1_prompt(self, imu_data):
        ...
        return self.system_prompt + f"""The IMU data is given in the IMU coordinate frame. The three-axis accelerations, gyroscopes and magnetometers are given below.
            Accelerations:
            x-axis: {imu_data.iloc[0]}; y-axis: {imu_data.iloc[1]}; z-axis: {imu_data.iloc[2]}
            Gyroscopes:
            x-axis: {imu_data.iloc[3]}; y-axis: {imu_data.iloc[4]}; z-axis: {imu_data.iloc[5]}
    ...
    def get_time_text_description_step2_prompt(self, imu_desc, imu_data, model):
        ...
        return self.system_prompt + f"""The person's action belongs to one of the following categories: Still, Walking, Run, Car. The IMU data is given in the IMU coordinate frame.
        ...
        The raw three-axis accelerations, gyroscopes and magnetometers are given below for your reference.
            ...
            Magnetometer:
            x-axis: {imu_data.iloc[6]}; y-axis: {imu_data.iloc[7]}; z-axis: {imu_data.iloc[8]}
            Could you please tell me what action the person was doing based on the given information, IMU description and IMU readings? Please make an analysis step by step. """ + self.get_response_format_prompt(model)

    def get_time_plot_prompt(self):
        """Generate prompt for IMU time series plot."""
        return self.system_prompt + f"""The three-axis accelerations, gyroscopes and magnetometers are plotted in the given plot.
            The person's action belongs to one of the following categories: Still, Walking, Run, Car.
            Could you please tell me what action the person was doing? Please make an analysis step by step.""" + self.get_response_format_prompt(model="gpt-4o")

    def get_time_plot_few_shot_prompt(self):
        ...
        return self.system_prompt + f"""The three-axis accelerations, gyroscopes and magnetometers are plotted in the given plot.
            ...
            Could you please tell me what action the person was doing based on the last plot? Please make an analysis step by step. """ + self.get_response_format_prompt(model="gpt-4o")
    ...
    def get_time_plot_env_prompt(self):
        ...
        return self.system_prompt + f"""You are given 2 images: the first image is the plot of three-axis accelerations, gyroscopes and magnetometers; the second image is an envorinment photo taken during this activity by a camera placed in the user's torso.
            The person's action belongs to one of the following categories: Still, Walking, Run, Car.
            Could you please tell me what action the person was doing based on the given information, IMU readings and environment photo? Please make an analysis step by step. """ + self.get_response_format_prompt(model="gpt-4o")
